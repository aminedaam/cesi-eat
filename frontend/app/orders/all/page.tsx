...
import { ShoppingBag, Clock, CheckCircle, XCircle, Loader2, ExternalLink } from "lucide-react";
    ...
    return (
      <div className="flex flex-col items-center justify-center min-h-screen p-4">
        <div className="bg-yellow-100 border border-yellow-400 text-yellow-700 px-4 py-3 rounded relative max-w-md" role="alert">
          ...
          <span className="block sm:inline"> Vous devez être connecté pour voir vos commandes.</span>
        </div>
      </div>
    );
  }
    ...
    <div className="container mx-auto px-4 py-8">
      ...
      {commandes.length === 0 ? (
        <div className="flex flex-col items-center justify-center p-8 bg-gray-50 rounded-lg shadow-sm">
          ...
          <p className="text-xl text-gray-600">Vous n'avez pas encore passé de commande</p>
          <p className="text-gray-500 mt-2">Explorez nos restaurants et commencez à commander!</p>
        </div>
      ) : (
                ...
                <div className="p-6 flex-grow flex flex-col">
                  ...
                  <div className="border-t border-gray-200 pt-4 mt-auto">
                    <div className="flex justify-between mb-2">
                      ...
                      <span className="font-semibold">{commande.prixTotal.toFixed(2)} €</span>
                    </div>
                    ...
                    <div className="mt-4">
                      ...
                      <ul className="space-y-1 max-h-40 overflow-y-auto">
                        ...
                        {commande.menu.map((item, index) => (
                          <li key={`menu-${index}`} className="text-sm text-gray-600 flex justify-between">
                            <span>{item.quantity}x {item.name}</span>
                            <span>{(item.price * item.quantity).toFixed(2)} €</span>
                          </li>
                        ))}
