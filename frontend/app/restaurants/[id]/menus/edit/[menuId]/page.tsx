...
import { getMenuByName, updateMenu } from "@/utils/apiMenu";
...
const EditMenuPage: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const fetchMenu = async () => {
      ...
      try {
        const fetchedMenu = await getMenuByName(String(menuId), accessToken);
        setInitialValues({
          name: fetchedMenu.name,
          description: fetchedMenu.description ?? "",
          priceMenu: fetchedMenu.priceMenu,
        });
      } catch (error) {
        console.error("Error fetching menu:", error);
        toast.error("Erreur lors de la récupération du menu.");
      }
    };
    fetchMenu();
  }, [accessToken, menuId]);
  ...
  const formik = useFormik<MenuFormValues>({
    ...
    onSubmit: async (values) => {
      ...
      try {
        ...
        await updateMenu(menuData.id, menuData, accessToken);
        ...
        setSubmitError(
          `Échec de la mise à jour du menu: ${
