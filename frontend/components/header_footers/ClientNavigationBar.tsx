...
export const ClientNavigationBar = ({ selectedPage }: { selectedPage?: string }) => {
  return (
    <div className="w-full h-16 bg-white items-center justify-around flex fixed bottom-0 z-50 border-t-2 border-gray-200">
      ...
      <Link href="/browse" className="flex flex-col items-center w-1/4">
        <Search className={selectedPage === "browse" ? "text-primary-50" : ""} />
        <span className={selectedPage === "browse" ? "text-primary-50" : ""} style={{ fontSize: "14px" }}>Parcourir</span>
      </Link>
      <Link href="/orders/all" className="flex flex-col items-center w-1/4">
        <Bookmark className={selectedPage === "orders" ? "text-primary-50" : ""} />
        <span className={selectedPage === "orders" ? "text-primary-50" : ""} style={{ fontSize: "14px" }}>Commandes</span>
      </Link>
